...
You can auto-generate this file by running the script with the `-g` flag, if you want. Note that the initial config generated by `-g` won't include the `log_file` field, but you can always add that if need be. 

The default configuration will be fine for most people, just make sure to create the folders defined below or specifically pass a location for the session and log files.

**Warning: You should create one of the `$HOME/.config/yams` or `$HOME/.yams` directories yourself. If not, the `session_file` (storing your saved session info) and log file will be saved in the current directory.**
